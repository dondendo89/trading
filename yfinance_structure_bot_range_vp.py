...
def build_double_message(
    ...
    title = "📣 DOPPIO MASSIMO (HH)" if kind == "HH" else "📣 DOPPIO MINIMO (LL)"
    parts = [
        title,
        f"💎 Asset: {symbol_name}",
...
def build_cross_message(symbol_name: str, kind: str, level: float, last_close: float, tv_symbol: str) -> str:
    if kind == "HH":
        return "\n".join(
            [
                "✅ Cross VAH dall'alto verso il basso dopo Doppio Massimo (HH)",
                f"💎 Asset: {symbol_name}",
    ...
    return "\n".join(
        [
            "✅ Cross VAL dal basso verso l'alto dopo Doppio Minimo (LL)",
            f"💎 Asset: {symbol_name}",
    ...
    for pivot, is_high in [(ph, True), (pl, False)]:
        ...
        if is_high and label == "HH":
            ...
            if state.prev_hh_price is not None and state.prev_hh_time is not None and pivot_price > state.prev_hh_price:
                poc, vah, val = volume_profile_between(df, state.last_hh_time, state.last_ll_time, num_rows, value_area_percent)
                send_telegram(
                    token,
                    chat_id,
