...
def send_alert(title, asset_name, direction, price, tv_sym, enabled):
    ...
    msg = (f"🚨 *{title}*\n\n💎 Asset: *{asset_name}*\n🔥 Segnale: {direction}\n💰 Prezzo: *{price:.4f}*\n\n🔗 [TradingView]({tv_url})")
    try: requests.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage", data={'chat_id': CHAT_ID, 'text': msg, 'parse_mode': 'Markdown', 'disable_web_page_preview': True})
    except: pass
...
def process_logic(df, df_dxy, sym_key, vah, val, cfg):
    ...
    # --- A. NUOVA LOGICA CROSS VAH/VAL (Sempre Attiva) ---
    curr_c, prev_c = c[-1], c[-2]
    # Cross VAH dall'alto verso il basso
    if prev_c > vah and curr_c < vah:
        send_alert("📉 VAH CROSS (Mean Reversion)", cfg['name'], "📉 SELL", curr_c, cfg['tv'], SEND_VOLUME_TOUCH)
    # Cross VAL dal basso verso l'alto
    if prev_c < val and curr_c > val:
        send_alert("📈 VAL CROSS (Mean Reversion)", cfg['name'], "📈 BUY", curr_c, cfg['tv'], SEND_VOLUME_TOUCH)
    ...
    if mem["pendHi"]:
        if c[-1] < mem["lastHiGrab"]: 
            send_alert("🔥 GRAB ↑ (Bearish Rejection)", cfg['name'], "📉 SELL", c[-1], cfg['tv'], SEND_GRAB); mem["pendHi"] = False
        else:
            mem["pendHiBars"] += 1
    ...
    if mem["pendLo"]:
        if c[-1] > mem["lastLoGrab"]: 
            send_alert("🔥 GRAB ↓ (Bullish Rejection)", cfg['name'], "📈 BUY", c[-1], cfg['tv'], SEND_GRAB); mem["pendLo"] = False
        else:
            mem["pendLoBars"] += 1
            if mem["pendLoBars"] > 3: mem["pendLo"] = False

# --- LOOP PRINCIPALE ---
print("🛰️ Test Telegram...")
send_alert("SISTEMA AGGIORNATO", "SYSTEM", "✅ Logica Cross VAH/VAL Attiva", 0, "DXY", True)
...
while True:
    try:
        ...
        for sym, cfg in ASSETS_CONFIG.items():
            print(f"[{t_now}] Analizzando: {cfg['name']}...", end="\r")
